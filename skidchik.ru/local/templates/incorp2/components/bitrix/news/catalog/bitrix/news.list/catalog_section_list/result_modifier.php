...
/**
 * Логика отбора категорий, привязанных к городу для данного шаблона.
...
//echo strtoupper(str_replace('/', '', $APPLICATION->GetCurPage())); // Название папки (promo, brands...) в верхнем для UF и нижнем для XML_ID 
//echo SITE_SERVER_NAME . "-" . str_replace('/', '', $APPLICATION->GetCurPage()); //обязательно добавляем к XML_ID и UF_GOROD_ название папки, где хранится каталог 

$rsEnum = CUserFieldEnum::GetList(array(), array('XML_ID' => SITE_SERVER_NAME . "-" . preg_replace('/\/(\w+)\/.*/', '$1', $APPLICATION->GetCurPage()) )); // USER_FIELD_NAME нет в документации, но работает
	$uf_prop = array();
