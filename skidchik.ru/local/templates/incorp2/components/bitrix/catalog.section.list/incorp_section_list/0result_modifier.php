...
{
	if  ($arSection [ "UF_TEST" ][ 0 ] != 'Поле для Мск' ){
...
/* foreach ($arResult['SECTIONS'] as $key => $arSection) {
	//printr($arSection);
	foreach ($arSection['UF_GOROD'] as $item) {
		if (($item == 70) && SITE_SERVER_NAME == 'skidchik.ru'){
			unset ($arResult['SECTIONS'][$key]);
		} 
	}
...
foreach ($uf_prop as $id => $xml_id) {
	if ($xml_id == SITE_SERVER_NAME) { // SITE_SERVER_NAME должен быть без скобок!!!
